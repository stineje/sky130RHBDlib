...
class extlvs():
	...
	def commandLineParser(self):
		...
		parser.add_argument('-s', '--source_directory', type=str, help='Sets the source directory for the schematic spice files.', default='/import/yukari1/lrburle/google_ring_oscillator/layout/xschem/spice/')
		parser.add_argument('-l', '--layout_directory', type=str, help='Sets the layout directory for the layout spice files.', default='/import/yukari1/lrburle/google_ring_oscillator/layout/ngspice/')
		...
		for f in self.files:
			if '.spice' in f and '.swp' not in f and 'fill' not in f:
				...
				try:
					...
					subout = subprocess.Popen([f'netgen -batch lvs \"{self.layout_path}{f} {f.split(".")[0]}\" \"{self.source_path}{f} {f.split(".")[0]}\" {self.pdk_root}/sky130A/libs.tech/netgen/sky130A_setup.tcl {f.split(".")[0]}.lvs.report'], shell=True) 
					subout.wait()
