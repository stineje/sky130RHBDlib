...
class ext_area_lef:
	def __init__(self):
		...
		print('These are the command line arguments:')
		print(self.args)
	...
	def commandLineParsing(self):
		...
		parser.add_argument("-lf", "--lib-file", help="Read in the lib file.", type=str, default=None)
		...
		with open(self.args.file, 'r') as f:
			...
			for line in lines:
				if 'FOREIGN' in line: 
					line = line.split(" ")
					cell = str(line[3])
				if ' SIZE' in line and len(cell) > 0:
					line = line.split(" ")
					print(line)
	...
	def readLibInsertArea(self, area):
		current_cell : str = ''
		with open(f'{self.args.lib_file}', 'r+') as f:
			lines = f.readlines()
			f.seek(0)

			for line in lines:
				if 'cell (' in line:
					current_cell = line.split('(')[1].split(')')[0]
				if 'area : ' in line:
					line = f'    area : {area[current_cell]};\n'
				
				f.write(line)

if __name__ == "__main__":
	area = ext_area_lef().addAreaLib()
	# ext_area_lef().readLibInsertArea(area)